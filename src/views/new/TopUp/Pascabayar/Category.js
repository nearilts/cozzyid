...
const Category = ({ setisShowed, setSelectedOperator,setbuttonPrice }) => {
    ...
    const handlePressOperator = (items) => {
        ...
        } else if (items.from_api === 'rajabiller')  {
            getPriceRajabiller(items)
        }else   {
    ...
    const getPriceRajabiller = async (items) => {
        const response = await axios.get(`${BASE_URL}produk-rajabiller`, {
            params: { category_id: 6, operator_id:items.id }
        });
    ...
    const fetchdigiflazz = async () => {
        const response = await axios.get(`${BASE_URL}productbyu-digiflazz`);
