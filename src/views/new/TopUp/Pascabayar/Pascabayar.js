...
const Pascabayar = ({ navigation }) => {
    ...
    const handlePesanClick = () => {
        ...
        }  else if (selectedOperator.from_api === 'rajabiller') {
            TransaksiRajabiller(phoneNumber,selectedOperator.code,CreditPoint)
            
        }
    };

    const TransaksiRajabiller = async (phoneNumber,selectedOperator,CreditPoint) => {
        let userInfo = await AsyncStorage.getItem('userInfo');
        userInfo = JSON.parse(userInfo);
        let token = userInfo.access_token.split('|')[1];
        
        const response = await axios.post(`${BASE_URL}transaction-rajabiller`, {
            kode: selectedOperator , idpel : phoneNumber , credit: CreditPoint
        }, {
            headers: {
        ...
        if (response.data.data.status === 'Gagal') {
            alert("Transaksi Gagal Tidak bisa di proses")
        } else {
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
            ...
            <View style={{ marginLeft: 10, flexDirection: "row", width: '100%', height: 50, backgroundColor: COLORS.white, marginTop: 40 }}>
                ...
                <View style={{ alignItems: 'center' }}>
                    <Text style={{ marginLeft: 140, marginTop: 5, fontSize: 18, color: COLORS.dark }}>Pascabayar</Text>
                </View>
            </View>
            ...
            <View style={styles.footer}>
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                    <Text style={styles.priceText}> label: {selectedPackage || 'Belum Dipilih'} </Text>
                </View>
