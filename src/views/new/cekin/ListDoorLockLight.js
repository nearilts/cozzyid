...
const ListDoorLockLight = ({ navigation }) => {
    ...
    const fetchProfil = async () => {
        try {
            ...
            console.log(`${url}listroomlight`);

            let token = userInfo.access_token.split('|')[1];
            const response = await axios.post(`${url}listroomlight`,{}, {
                headers: {
    ...
    const checkout = async (id) => {
        ...
        navigation.navigate('OnOfLight', id); 
    };
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', padding: 10, alignItems: 'center' }}>
                <View style={{ flexDirection: 'column', padding: 5 }}>
                    <Text style={{ color: COLORS.dark, fontSize: 18 }}>
                       No  : {item.light.device_name}
                    </Text>
                    <Text style={{ color: COLORS.grey, fontSize: 13 }}>
                        Location : {item.light.name}
                    </Text>
                    <Text style={{ color: COLORS.grey, fontSize: 13 }}>
...
export default ListDoorLockLight;
